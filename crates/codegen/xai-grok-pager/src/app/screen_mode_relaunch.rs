...
pub(crate) fn screen_mode_relaunch_resume_hint(session_id: &str, want_minimal: bool) -> String {
    ...
    format!("{GROK_SCREEN_MODE_ENV}={mode} gcode {flag} --resume {session_id}")
}
...
mod tests {
    ...
    fn failed_relaunch_hint_includes_screen_mode_env() {
        ...
        assert_eq!(
            ...
            "GROK_SCREEN_MODE=fullscreen gcode --fullscreen --resume abc-sid"
        );
        assert_eq!(
            screen_mode_relaunch_resume_hint("abc-sid", true),
            "GROK_SCREEN_MODE=minimal gcode --minimal --resume abc-sid"
        );
    }
