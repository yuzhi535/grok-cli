...
const MSG_RUN_UPDATE_MANUAL: &str = "Run `gcode update` to get the latest version.";
...
fn reinstall_hint(installer: &str, channel: &str) -> String {
    match installer {
        ...
        "gh-release" => "Please reinstall via GitHub Releases:\n  gh release download --repo yuzhi535/gcode --pattern 'gcode-*'".to_string(),
...
pub fn print_update_status(status: &UpdateStatus, json: bool) -> anyhow::Result<()> {
    ...
    if let Some(error) = status.error.as_deref() {
        println!("Gcode - v{} [{}]", status.current_version, status.channel);
    ...
    if status.update_available {
        if let Some(latest_version) = status.latest_version.as_deref() {
            println!(
                "A new version of Gcode is available: {} -> {}{}",
                status.current_version, latest_version, channel_label
            );
        } else {
            println!("A new version of Gcode is available.");
        }
        return Ok(());
    }
    ...
    println!("Gcode - v{}{}", status.current_version, channel_label);
    Ok(())
}
...
/// Download and install grok from GitHub Releases (yuzhi535/grok-cli).
///
