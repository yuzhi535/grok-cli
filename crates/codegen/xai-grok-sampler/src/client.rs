...
impl EndpointTemplate {
    ...
    /// Render an already-resolved endpoint while preserving any configured
    /// query suffix from the original base URL.
    fn url_for_resolved_endpoint(&self, endpoint: &str) -> String {
        match self {
            Self::Plain(_) => endpoint.to_string(),
            Self::WithQuery { suffix, .. } => format!("{endpoint}{suffix}"),
        }
    }
}
...
impl SamplingClient {
    ...
    fn endpoint(&self, path: &str) -> String {
        ...
        {
            return self.endpoint.url_for_resolved_endpoint(&url);
        }
